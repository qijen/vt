...
public:
	xQueue();
	~xQueue();
	xQueue( xQueue& original_copy );
	bool operator= ( const xQueue& original_copy );
...
template <class queue_type>
	inline unsigned int xQueue<queue_type>::Size( void ) const
{
	if ( head <= tail )
		return tail -head;
	else
...
template <class queue_type>
inline bool xQueue<queue_type>::IsEmpty(void) const
{
	return head==tail;
}

template <class queue_type>
inline unsigned int xQueue<queue_type>::AllocationSize( void ) const
{
	return allocation_size;
}

template <class queue_type>
	xQueue<queue_type>::xQueue()
{
	allocation_size = 16;
...
template <class queue_type>
	inline queue_type xQueue<queue_type>::Pop( void )
{
#ifdef _DEBUG
...
template <class queue_type>
	void xQueue<queue_type>::PushAtHead( const queue_type& input, unsigned index )
{
	assert(index <= Size());
...
template <class queue_type>
	inline queue_type xQueue<queue_type>::Peek( void ) const
{
#ifdef _DEBUG
	assert( head != tail );
...
template <class queue_type>
	inline queue_type xQueue<queue_type>::PeekTail( void ) const
	{
#ifdef _DEBUG
		assert( head != tail );
		assert( allocation_size > 0 && Size() >= 0 );
...
template <class queue_type>
	void xQueue<queue_type>::Push( const queue_type& input )
{
	if ( allocation_size == 0 )
	{
...
template <class queue_type>
	xQueue<queue_type>::xQueue( xQueue& original_copy )
{
...
template <class queue_type>
	bool xQueue<queue_type>::operator= ( const xQueue& original_copy )
{
...
template <class queue_type>
	inline void xQueue<queue_type>::Clear ( void )
{
	if ( allocation_size == 0 )
		return ;
...
template <class queue_type>
	void xQueue<queue_type>::Compress ( void )
{
	queue_type* new_array;
...
template <class queue_type>
	bool xQueue<queue_type>::Find ( queue_type q )
{
	if ( allocation_size == 0 )
		return false;
...
template <class queue_type>
	void xQueue<queue_type>::ClearAndForceAllocation( int size )
{
	delete [] array;
...
template <class queue_type>
	inline queue_type& xQueue<queue_type>::operator[] ( unsigned int position ) const
{
#ifdef _DEBUG
...
template <class queue_type>
void xQueue<queue_type>::RemoveAtIndex( unsigned int position )
{
#ifdef _DEBUG
	assert( position < Size() );
	assert( head != tail );
