...
CKObjectDeclaration	*FillBehaviorNSGetStateDecl();
CKERROR CreateNSGetStateProto(CKBehaviorPrototype **);
int NSGetState(const CKBehaviorContext& behcontext);
CKERROR NSGetStateCB(const CKBehaviorContext& behcontext);
...
CKObjectDeclaration	*FillBehaviorNSGetStateDecl()
{
	CKObjectDeclaration *od = CreateCKObjectDeclaration("NSGetState");	
	od->SetDescription("Returns the session status");

	od->SetCategory("TNL/Sessions");
	od->SetType( CKDLL_BEHAVIORPROTOTYPE);
	od->SetGuid(CKGUID(0x1c0222e1,0x27bf1884));
	od->SetAuthorGuid(VIRTOOLS_GUID);
	od->SetAuthorName(VTCX_AUTHOR);
	od->SetVersion(0x00010000);
	od->SetCreationFunction(CreateNSGetStateProto);
	od->SetCompatibleClassId(CKCID_OBJECT);
	return od;
}


CKERROR CreateNSGetStateProto(CKBehaviorPrototype **pproto)
{
	CKBehaviorPrototype *proto = CreateCKBehaviorPrototype("NSGetState");
	...
	proto->SetFunction(NSGetState);
	proto->SetBehaviorCallbackFct(NSGetStateCB);
