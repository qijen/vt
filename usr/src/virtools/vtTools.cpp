...
int vtTools::ParameterTools::IsNumeric(const char* Str,vtTools::Enums::SuperType superType)
{
...
* Parameters :	CKContext* context, r : the virtools context.
						...
						uxString stringValue,r : the string value of the given type.
		...
		case vtENUMERATION:
			{
				...
				if ( enumStruct )
				{
					for (int i = 0 ; i < enumStruct->GetNumEnums() ; i ++ )
					{
						...
						item->value <<enumStruct->GetEnumValue(i);

						item->label =enumStruct->GetEnumDescription(i);
					}
				}
			break;
			}
