...
CSplashScreenEx::CSplashScreenEx()
{
	...
	m_uTextFormat=DT_VCENTER | DT_VCENTER | DT_WORDBREAK;
...
void CSplashScreenEx::Show()
{
	...
	{
		m_fnAnimateWindow(m_hWnd,m_FadeInTime,AW_BLEND);
	}
	else
		ShowWindow(SW_SHOW);
	
	if (m_FadeInTime!=0)
		SetTimer(0,m_FadeInTime,NULL);
}

void CSplashScreenEx::Hide()
{
	if ((m_dwStyle & CSS_FADEOUT) && (m_fnAnimateWindow!=NULL))
		m_fnAnimateWindow(m_hWnd,m_FadeOutTime,AW_HIDE | AW_BLEND);
	else
		ShowWindow(SW_HIDE);
