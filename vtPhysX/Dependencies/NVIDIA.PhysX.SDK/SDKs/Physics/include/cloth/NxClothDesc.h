...
NX_INLINE bool NxClothSplitPairData::isValid() const
	{
...
NX_INLINE bool NxClothConstrainCoefficients::isValid() const
	{
...
{
	if (flags & NX_CLF_INTER_COLLISION) return false;	// not supported on any platform yet

	if(!clothMesh) return false;
	if(!globalPose.isFinite()) return false;
	if(thickness < 0.0f) return false;
	if(selfCollisionThickness <= 0.0f) return false;
	...
	if(!splitPairData.isValid()) return false;
