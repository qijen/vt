...
/*		errMessage.Format("autobreak");*/
	...
	if ( calculateBreaking )
	{
		...
		if ( (getBreakFlags() & VBF_UseTable) )
		{
			...
			{
				bTorque = getBrakeAmountFromTable(BL_Small);
				errMessage.Format("breaking at small : bt : %f at %f",bTorque,mTimeBreaking);
				//xInfo(errMessage.Str());

			}else if(mTimeBreaking >= mBrakeMediumThresold && mTimeBreaking < mBrakeHighThresold )
			{
				bTorque = getBrakeAmountFromTable(BL_Medium);
				errMessage.Format("breaking at medium : bt : %f at %f",bTorque,mTimeBreaking);
				//xInfo(errMessage.Str());
			}else
			{
				bTorque = getBrakeAmountFromTable(BL_High);
				errMessage.Format("breaking at high : bt : %f at %f",bTorque,mTimeBreaking);
				//xInfo(errMessage.Str());
			}
		}else
		{
			...
			if(mTimeBreaking < mBrakeMediumThresold)
			{
				bTorque = mCurrentBrakeTorque * mBreakPressures[BL_Small];
				errMessage.Format("breaking at small : bt : %f at %f",bTorque,mTimeBreaking);
				//xInfo(errMessage.Str());
			...
			{
				bTorque = mCurrentBrakeTorque * mBreakPressures[BL_Medium];
				errMessage.Format("breaking at medium : bt : %f at %f",bTorque,mTimeBreaking);
				//xInfo(errMessage.Str());
			}
			else
			{
				bTorque = mCurrentBrakeTorque * mBreakPressures[BL_High];
				errMessage.Format("breaking at high : bt : %f at %f",bTorque,mTimeBreaking);
				//xInfo(errMessage.Str());
			}
	...
	for(NxU32 i = 0; i < _wheels.size(); i++) 
	{
		...
		pWheel* wheel = _wheels[i];
		wheel->tick( ( _currentStatus & VS_Handbrake) , motorTorque, bTorque  , dt );
