...
pVehicle *pFactory::createVehicle(CK3dEntity *body,pVehicleDesc descr)
{
	...
	descr.body = rBody;
	result  = new pVehicle(descr);

//	result->_vehicleMotor  = createVehicleMotor(*descr.getMotorDescr());
//result->_vehicleGears = createVehicleGears(*descr.getGearDescription());

	result->_vehicleMotor  = NULL;
	result->_vehicleGears = NULL;

	int wSize = result->getWheels().size();
