...
int registerJBall(CK3dEntity *target,int attributeType,bool set,bool isPostJob)
{
	...
	if (distanceParameter )
	{
		...
		if (set)
		{
			...
			VxVector axis  = GetValueFromParameterStruct<VxVector>(distanceParameter,PS_JBALL_GLOBAL_AXIS);
			CK_ID axisRefID = GetValueFromParameterStruct<CK_ID>(distanceParameter,PS_JBALL_GLOBAL_AXIS_REF);
			CK3dEntity *axisRef = static_cast<CK3dEntity*>(GetPMan()->m_Context->GetObject(axisRefID));
			if (axisRef)
			{
				VxVector dir,up,right;
			...
			pJointBall *joint   = static_cast<pJointBall*>(pFactory::Instance()->createBallJoint(target,bodyBEnt,anchor0,swingAxis,axis));
			if (joint)
			{
...
int registerJDistance(CK3dEntity *target,int attributeType,bool set,bool isPostJob)
{
	...
	if(!GetPMan()->GetContext()->IsPlaying() && !GetPMan()->isValid() )
		return 0;
