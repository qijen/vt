...
void PhysicManager::_RegisterVehicleParameters()
{
	...
	pm->RegisterNewStructure(VTS_VGEAR_RATIO_ENTRY,"Ratio/Inertia","Ratio,Inertia",CKPGUID_FLOAT,CKPGUID_FLOAT);
	param_type=pm->GetParameterTypeDescription(VTS_VGEAR_RATIO_ENTRY);
	if (param_type) param_type->dwFlags|=CKPARAMETERTYPE_HIDDEN;
	pm->RegisterNewStructure(VTS_VGEAR_RATIOS,"pVehicleGear List","N,R,1,2,3,4,5,6,7",
		VTS_VGEAR_RATIO_ENTRY,
		VTS_VGEAR_RATIO_ENTRY,		
		VTS_VGEAR_RATIO_ENTRY,
		VTS_VGEAR_RATIO_ENTRY,		
		VTS_VGEAR_RATIO_ENTRY,
	...
	pm->RegisterNewEnum(VTE_XML_VGEAR_SETTINGS,"pVehicleGearSettings","Stub=1");

	
	pm->RegisterNewFlags(VTS_VGEARBOX_FLAGS,"pVehicleGearBoxFlags","Stub=1");
	pm->RegisterNewFlags(VTF_VEHICLE_ENGINE_FLAGS,"pVehicleEngineFlags","Stub=1");
	...
	pm->RegisterNewFlags(VTF_VFLAGS,"pVehicleFlags","Use Advanced=1");
	...
	pm->RegisterNewFlags(VTF_VWSHAPE_FLAGS,"pWheelShapeFlags","AxisContactNormal=1,InputLateralSlip=2,InputLongitudinal=4,UnscaledSpringBehavior=8,AxleSpeedOverride=16,EmulateLegacyWheel=32,ClampedFriction=64");
