...
pRigidBody*pFactory::createRigidBody(CK3dEntity *referenceObject,pObjectDescr& oDescr)
{
	...
	if ( !GetPMan()->isValid()  )
	{
		...
		{
			GetPMan()->copyToAttributes(oDescr,referenceObject);
			return NULL;
		}
	}
...
/*	if (!world)
	{
		xLogger::xLog(XL_START,ELOGERROR,E_LI_MANAGER,"world object still invalid, vtPhysX corrupt. Performing initial setup");
		if (!GetPMan()->isValid()){
...
/*	if (oDescr.skinWidth < 0.0f)
		oDescr.skinWidth = 0.025f;
*/
	if (oDescr.skinWidth <= 0.001f)
		oDescr.skinWidth = 0.025f;

	if (oDescr.collision.skinWidth <= 0.001f)
		oDescr.collision.skinWidth = 0.025f;
	...
	if (oDescr.skinWidth == 0.0f)
	{
		oDescr.skinWidth = 0.25f;
	}
	
	if (oDescr.collision.skinWidth == 0.0f)
	{
		oDescr.collision.skinWidth = 0.25f;
	}
	*/
	...
	case HT_Box:
		{
			...
			if(world->getCompartment())
			{
				actorDesc.compartment = world->getCompartment();
			}
			break;
		}
		//////////////////////////////////////////////////////////////////////////
	...
	case HT_Mesh:
		{
			...
			myMesh.setToDefault();
			if (result->getWorld()->getCompartment())
			{
				myMesh.flags |=NX_MF_HARDWARE_MESH;
			}
	...
	result->enableCollisionsNotify((result->getFlags() & BF_CollisionNotify));
	result->enableCollisionForceCalculation((result->getFlags() & BF_CollisionsForce), referenceObject);
	result->enableContactModification((result->getFlags() & BF_ContactModify));
	...
	CKSTRING name  = referenceObject->GetName();


	result->setSleeping( (result->getFlags() & BF_Sleep));

	if (oDescr.flags & BF_AddAttributes)
	{
		GetPMan()->copyToAttributes(oDescr,referenceObject);
	}
	...
	while (subEntity= referenceObject->HierarchyParser(subEntity) )
	{
		...
		/************************************************************************/
		/* new rigid body in hierarchy                                                                     */
		/************************************************************************/
		if ( !(subDescr->flags & BF_SubShape) && !result->isSubShape(subEntity)  )
		{
			CKSTRING name  = referenceObject->GetName();
			CKSTRING namesub  = subEntity->GetName();
			pRigidBody *childBody = GetPMan()->getBody(subEntity);
			if (!childBody)
				childBody= pFactory::Instance()->createRigidBody(subEntity,*subDescr);
		}
		...
		if (subDescr->flags & BF_SubShape )
		{
			...
			if (subDescr->hullType != HT_Cloth)
			{
				...
				NxShape *sC = GetPMan()->getSubShape(subEntity);
				if (!sC)
				{
				...
				}else
				{
					...
					pSubMeshInfo *sInfo = static_cast<pSubMeshInfo*>(sC->userData);
					if (sInfo)
					{

					}
				
					NxActor *sActor = &sC->getActor();
					if (sActor)
					{
						pRigidBody *sBody = static_cast<pRigidBody*>(sActor->userData);
						if (sBody)
						{
