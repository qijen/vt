...
//			((pWheel2*)wheel)->setEntity(static_cast<CK3dEntity*>(GetPMan()->GetContext()->GetObject(srcReference->GetID())));
...
int pFactory::copyTo(CKParameterOut *dst,const pWheelContactData& src)
{
	...
	SetParameterStructureValue<float>(dst,E_WCD_LAT_SLIP,src.lateralSlip,false);

	SetParameterStructureValue<float>(dst,E_WCD_LONG_IMPULSE,src.longitudalImpulse,false);
	SetParameterStructureValue<float>(dst,E_WCD_LAT_IMPULSE,src.lateralImpulse,false);
